...
```python
mycols = ['years', 'hrs1', 'educ', 'sei', 'income', 'occup', 'wrkstat', 'class', 'satjob', 'finalter']
df_gss_sample_ten_cols = df_gss_sample[mycols]

## my variables are related to one another for many reasons.  Years of education is connected to income and hours they work per week.  
## Income effects socioeconomic index and social class.  Job satisfaction is connected to employment status and income.  Financial situation 
## is connected to income, hours worked per week, social class, etc. All of these variables are interlinked and build off of one another.
```
...
## how to display result as a data frame???
```
...
sns.scatterplot(data = df_gss_sample, x = 'realinc', y = 'prestg80', alpha = 1)
